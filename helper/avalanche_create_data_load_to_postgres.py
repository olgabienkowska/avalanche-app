...
def generate_avalanche_data(num_records=100):
...
def store_in_postgresql(df, table_name, db_name, user, password, host=db_host, port=5432):
    # Create a connection string
    engine = create_engine(f"postgresql+psycopg2://{user}:{password}@{host}:{port}/{db_name}")
    df.to_sql(table_name, engine, if_exists='replace', index=False)

# Uncomment if you want to generate the data and store in postgres db.
# table_df = generate_avalanche_data(100)
# store_in_postgresql(table_df, table_name="avalanche_incidents", db_name="avalanche_app", user="postgres", password=password_db)
